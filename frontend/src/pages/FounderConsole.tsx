...
const FounderConsole = () => {
  ...
  useEffect(() => {
    const fetchPolicies = async () => {
      try {
        ...
        const corePolicies: Policy[] = (coreRes || []).map((p: any) => ({
          id: p.id,
          name: p.name,
          category: p.category,
          type: 'CORE'
        ...
        const customPolicies: Policy[] = (customRes || []).map((p: any) => ({
          id: p.id,
          name: p.name,
          type: 'CUSTOM'
        }));
  ...
  const copySocialReport = () => {
    ...
    const text = `🛡️ Zaxion Deep Audit Report: ${results.owner}/${results.repo}\n\n` +
      `📊 Security Grade: ${results.summary.grade} (${results.summary.score}%)\n` +
      `🔍 PRs Analyzed: ${results.totalAnalyzed}\n` +
      `✅ Passed: ${results.summary.passed}\n` +
      `❌ Blocked: ${results.summary.blocked}\n` +
      `⚠️ Warnings: ${results.summary.warned}\n\n` +
      `Generated by Zaxion Platform | Automated Governance for Modern Teams\n` +
      `#Zaxion #GitHub #OpenSource #AppSec`;
      
    navigator.clipboard.writeText(text);
    toast.success("Social report copied to clipboard!");
  };
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground font-mono">
                          Score: {results.summary.score}% • {results.totalAnalyzed} PRs
                        </p>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-green-500/5 border border-green-500/10 p-3 rounded-xl text-center">
                    <p className="text-[10px] font-bold text-green-500/60 uppercase">Passed</p>
                    <p className="text-xl font-black text-green-500">{results.summary.passed}</p>
                  </div>
                  <div className="bg-destructive/5 border border-destructive/10 p-3 rounded-xl text-center">
                    <p className="text-[10px] font-bold text-destructive/60 uppercase">Blocked</p>
                    <p className="text-xl font-black text-destructive">{results.summary.blocked}</p>
                  </div>
                  <div className="bg-amber-500/5 border border-amber-500/10 p-3 rounded-xl text-center">
                    <p className="text-[10px] font-bold text-amber-500/60 uppercase">Warned</p>
                    <p className="text-xl font-black text-amber-500">{results.summary.warned}</p>
                  </div>
                </div>
